...
export class FieldMappingUpdateTool extends Tool<typeof InputSchema> {
  ...
  description = "Updates one or more field mappings' source field, primary key flag, normalizedKey, required, defaultValue, format, or enumValues. Accepts 1–100 items.";
        ...
        try {
          ...
          await Repository.transaction(async (tx) => {
            for (const item of items) {
              ...
              await DbService.repository.fieldMappings.update(fieldMappingId, updateData as never, tx);
            }
          });
          ...
          return {
            ...
            items: items.map((item) => ({
              ...
              summary: { sourceField: mappings[item.fieldMappingId]?.sourceField },
            })),
          };
        } catch (err: unknown) {
          const message = err instanceof Error ? err.message : "Failed to update field mappings";
