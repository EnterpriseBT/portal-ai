...
describe("swagger spec — document completeness (#420)", () => {
  ...
  // Real spec conformance (#446). Until the document declared 3.1 this was
  // impossible: 59 components are generated by `z.toJSONSchema`, which emits
  // JSON Schema draft 2020-12, and 3.0 permits none of it — 1928 errors.
  //
  // This does NOT replace the shape check above, which was the assumption
  // going in. Injecting a collapsed flow map (a property named "{ type:
  // string }" whose value is null) leaves this assertion green: unrecognized
  // 2020-12 keywords may hold any value, so garbled-but-legal survives
  // conformance. The two checks cover disjoint failures.
  it("validates against the OpenAPI specification", async () => {
    // `swaggerSpec` is typed `object` by swagger-jsdoc; the clone keeps
    // validate() from mutating the module-level document other tests read.
    const result = await validate(
      structuredClone(swaggerSpec) as Parameters<typeof validate>[0]
    );

    // Surface the messages, not just `false` — a bare boolean makes this the
    // least actionable failure in the suite.
    expect(
      result.valid
        ? []
  ...
  // 3.1 removed `nullable` in favour of `type: [X, "null"]`, and 2020-12
  // ignores keywords it does not recognize — so a reintroduced `nullable` is
  // silently inert rather than invalid, and the conformance check above cannot
  // see it. Copying a 3.0-shaped annotation from elsewhere is the likely way
  // back in, which is why this is asserted separately.
  it("expresses nullability the 3.1 way, never with `nullable`", () => {
    ...
    const walk = (node: unknown, path: string): void => {
      ...
      for (const [key, value] of Object.entries(node)) {
        if (key === "nullable") offenders.push(path || "<root>");
        walk(value, `${path}.${key}`);
      }
    };
