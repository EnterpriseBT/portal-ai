...
const mockFindEntityById = jest
  .fn<any>()
  .mockResolvedValue({ id: "ce-1", key: "contacts", label: "My Entity" });

jest.unstable_mockModule(
  "../../services/connector-entity-validation.service.js",
  () => ({
    ConnectorEntityValidationService: {
...
const exec = (input: { items: Item[] }) =>
  new ConnectorEntityDeleteTool().build("station-1", "user-1").execute!(input, {
    toolCallId: "t",
    messages: [],
...
describe("ConnectorEntityDeleteTool", () => {
  ...
  it("validation failure — one entity fails validateDelete, nothing deleted", async () => {
    ...
    const result: any = await exec({
      items: [{ connectorEntityId: "ce-1" }, { connectorEntityId: "ce-2" }],
    });
