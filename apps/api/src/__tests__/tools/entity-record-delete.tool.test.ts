...
const mockFindRecordById = jest
  .fn<any>()
  .mockResolvedValue({ id: "rec-1", connectorEntityId: "ce-1" });
  ...
  DbService: {
    repository: {
      entityRecords: {
        ...
        softDeleteMany: mockSoftDeleteMany,
      },
...
const exec = (input: { items: ItemInput[] }) =>
  new EntityRecordDeleteTool().build("station-1", "user-1").execute!(input, {
    toolCallId: "t",
    messages: [],
...
describe("EntityRecordDeleteTool", () => {
  it("single-item regression — soft-deletes the record", async () => {
    ...
    expect(mockSoftDeleteMany).toHaveBeenCalledWith(
      ["rec-1"],
      "user-1",
      "mock-tx"
    );
  ...
  it("returns error when write check fails", async () => {
    mockAssertWriteCapability.mockRejectedValueOnce(
      new Error("Write disabled")
    );
