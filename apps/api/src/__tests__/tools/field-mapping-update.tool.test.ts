...
const mockFindById = jest.fn<() => Promise<unknown>>().mockResolvedValue({
  ...
  columnDefinitionId: "cd-1", isPrimaryKey: false,
});
...
type Item = { fieldMappingId: string; sourceField?: string; isPrimaryKey?: boolean; normalizedKey?: string; required?: boolean; defaultValue?: string | null; format?: string | null; enumValues?: string[] | null };
...
describe("FieldMappingUpdateTool", () => {
  it("single-item regression — updates sourceField", async () => {
    const result: any = await exec({ items: [{ fieldMappingId: "fm-1", sourceField: "Full Name" }] });
    expect(result.success).toBe(true);
    expect(result.count).toBe(1);
    expect(result.items).toHaveLength(1);
    expect(mockUpdate).toHaveBeenCalledWith("fm-1", expect.objectContaining({ sourceField: "Full Name" }), "mock-tx");
  });

  it("bulk update — 3 mappings updated in transaction", async () => {
    mockFindById
      .mockResolvedValueOnce({ id: "fm-1", connectorEntityId: "ce-1", organizationId: "org-1", sourceField: "A", columnDefinitionId: "cd-1", isPrimaryKey: false })
      .mockResolvedValueOnce({ id: "fm-2", connectorEntityId: "ce-1", organizationId: "org-1", sourceField: "B", columnDefinitionId: "cd-2", isPrimaryKey: false })
      .mockResolvedValueOnce({ id: "fm-3", connectorEntityId: "ce-1", organizationId: "org-1", sourceField: "C", columnDefinitionId: "cd-3", isPrimaryKey: false });
  ...
  it("validation failure — mapping not found", async () => {
    ...
    const result: any = await exec({ items: [{ fieldMappingId: "fm-missing", sourceField: "X" }] });
  ...
  it("validation failure — wrong organization", async () => {
    mockFindById.mockResolvedValueOnce({ id: "fm-1", connectorEntityId: "ce-1", organizationId: "other-org", sourceField: "A" });

    const result: any = await exec({ items: [{ fieldMappingId: "fm-1", sourceField: "X" }] });
