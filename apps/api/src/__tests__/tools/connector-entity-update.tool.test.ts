...
describe("ConnectorEntityUpdateTool", () => {
  it("single-item regression — updates entity label", async () => {
    const result: any = await exec({ items: [{ connectorEntityId: "ce-1", label: "New Label" }] });
    expect(result.success).toBe(true);
    expect(result.count).toBe(1);
    expect(result.items).toHaveLength(1);
    expect(mockUpdate).toHaveBeenCalledWith("ce-1", expect.objectContaining({ label: "New Label", updatedBy: "user-1" }), "mock-tx");
  });

  it("bulk update — 3 entities updated in transaction", async () => {
    mockFindById
      .mockResolvedValueOnce({ id: "ce-1", key: "a", label: "A", connectorInstanceId: "ci-1" })
      .mockResolvedValueOnce({ id: "ce-2", key: "b", label: "B", connectorInstanceId: "ci-1" })
      .mockResolvedValueOnce({ id: "ce-3", key: "c", label: "C", connectorInstanceId: "ci-1" });
  ...
  it("validation failure — entity not found", async () => {
    ...
    const result: any = await exec({ items: [{ connectorEntityId: "ce-missing", label: "X" }] });
  ...
  it("scope check failure blocks all items for that entity", async () => {
    ...
    const result: any = await exec({ items: [{ connectorEntityId: "ce-1", label: "X" }] });
