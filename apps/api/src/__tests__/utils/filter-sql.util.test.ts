...
function flattenChunks(sqlObj: SQL): unknown[] {
  ...
  for (const chunk of (sqlObj as unknown as { queryChunks: unknown[] })
    .queryChunks) {
    if (
      chunk &&
      typeof chunk === "object" &&
  ...
  it("should sanitize field keys (SQL injection in field name)", () => {
    ...
    const msg = expectError(
      encode({
        combinator: "and",
        conditions: [
          { field: "name'; DROP TABLE--", operator: "eq", value: "x" },
        ],
      })
    );
    expect(msg).toContain("Unknown field");
        ...
        conditions: [
          {
            field: "name",
            operator: "eq",
            value: "'; DROP TABLE entity_records; --",
          },
        ],
      })
    );
...
describe("parseAndBuildFilterSQL — Phase 4", () => {
  ...
  it("filter field keys reference normalizedData JSONB keys", () => {
    ...
    expect(
      chunks.some((c) => typeof c === "string" && c.includes("'name'"))
    ).toBe(true);
  });
