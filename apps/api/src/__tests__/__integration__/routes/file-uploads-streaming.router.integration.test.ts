...
describe("File uploads streaming router", () => {
  ...
  describe("POST /api/file-uploads/parse-session", () => {
    ...
    it("merges multi-CSV uploads into one workbook with preview cells inline", async () => {
      ...
      const res = await request(app)
        .post("/api/file-uploads/parse-session")
        .set("Authorization", "Bearer test-token")
        .send({ uploadIds });
      expect(res.status).toBe(200);
  ...
  describe("Layout plans /interpret + /commit with uploadSessionId", () => {
    ...
    it("rejects interpret when both uploadSessionId and workbook are supplied", async () => {
      const res = await request(app)
        .post("/api/layout-plans/interpret")
        .set("Authorization", "Bearer test-token")
        .send({
          uploadSessionId: "s_any",
          workbook: { sheets: [{ name: "S", dimensions: { rows: 0, cols: 0 }, cells: [] }] },
          regionHints: [],
        });
      expect(res.status).toBe(400);
