...
function createConnectorInstance(connectorDefinitionId: string, organizationId: string) {
  return {
    id: generateId(),
    connectorDefinitionId, organizationId,
    name: "Test Instance",
    status: "active" as const,
    config: null, credentials: null,
    lastSyncAt: null, lastErrorMessage: null,
...
function createConnEntity(organizationId: string, connectorInstanceId: string) {
  return {
    id: generateId(), organizationId, connectorInstanceId,
...
function createSyncRecord(
  ...
  return {
    id: generateId(), organizationId, connectorEntityId,
    data, normalizedData,
...
async function seedStation(db: ReturnType<typeof drizzle>) {
  const { userId, organizationId } = await seedUserAndOrg(db, "auth0|sync-test");
  ...
  const station = {
    id: generateId(), organizationId, name: "Sync Station",
    description: null, toolPacks: ["entity_management"],
    created: now, createdBy: "SYSTEM_TEST",
    updated: null, updatedBy: null, deleted: null, deletedBy: null,
  };
  await db.insert(stations).values(station as never);
  await db.insert(stationInstances).values({
    id: generateId(), stationId: station.id, connectorInstanceId: inst.id,
    created: now, createdBy: "SYSTEM_TEST",
    updated: null, updatedBy: null, deleted: null, deletedBy: null,
  } as never);
  ...
  return { userId, organizationId, stationId: station.id, connectorEntityId: entity.id, connectorInstanceId: inst.id };
}
...
describe("Sync-after-mutation interaction", () => {
  ...
  it("sync does not overwrite tool-created records (UUID sourceId vs row-index sourceId)", async () => {
    ...
    const [portalRow] = await db.select().from(entityRecords).where(eq(entityRecords.id, portalRecordId));
  ...
  it("sync restores tool-deleted synced records", async () => {
    ...
    const syncRec = createSyncRecord(s.organizationId, s.connectorEntityId, "0", { Name: "Original" }, { name: "Original" });
    ...
    const restoredRec = createSyncRecord(s.organizationId, s.connectorEntityId, "0-restored", { Name: "Restored" }, { name: "Restored" });
    await db.insert(entityRecords).values(restoredRec as never);

    // The restored record should exist
    const [restored] = await db.select().from(entityRecords).where(eq(entityRecords.id, restoredRec.id));
  ...
  it("sync overwrites tool-modified synced records", async () => {
    ...
    // Insert a synced record
    const syncRec = createSyncRecord(s.organizationId, s.connectorEntityId, "row-0", { Name: "Original" }, { name: "Original" });
    await db.insert(entityRecords).values(syncRec as never);
    ...
    await db.update(entityRecords)
      .set({ data: { Name: "Original" }, normalizedData: { name: "Original" }, checksum: "new-checksum" } as never)
      .where(eq(entityRecords.id, syncRec.id));

    const [row] = await db.select().from(entityRecords).where(eq(entityRecords.id, syncRec.id));
  ...
  it("sync uses tool-created field mappings (new mapping applied to normalizedData)", async () => {
    ...
    const newMapping = createFieldMapping(s.organizationId, s.connectorEntityId, newCol.id, "Email");
