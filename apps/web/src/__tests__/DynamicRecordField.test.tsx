...
const { DynamicRecordField } =
  await import("../components/DynamicRecordField.component");
...
describe("DynamicRecordField — rendering by type", () => {
  ...
  it("renders type=datetime-local input for datetime type", () => {
    render(
      <DynamicRecordField
        column={col("ts", "datetime")}
        value=""
        onChange={noop}
      />
    );
    expect(screen.getByLabelText("Ts")).toHaveAttribute(
      "type",
      "datetime-local"
    );
  });
  ...
  it("renders multiline field for reference-array type", () => {
    render(
      <DynamicRecordField
        column={col("refs", "reference-array")}
        value=""
        onChange={noop}
      />
    );
