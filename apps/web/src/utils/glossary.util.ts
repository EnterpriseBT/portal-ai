...
export const GLOSSARY_ENTRIES: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: GlossaryCategory.Analytics,
    definition:
      "A per-toolpack HMAC secret (`whsec_*` prefix, 32 random bytes base64url-encoded) used to sign every outbound webhook call. Generated server-side at registration and shown to you exactly once — copy it into your toolpack server's environment immediately. The secret is encrypted at rest in Portal.ai's database; subsequent reads only confirm presence (`{has: true}`), never plaintext. To re-view a secret, rotate it from the toolpack's edit dialog: a fresh value is generated, the old one is invalidated immediately, and the new plaintext is shown once. Every outbound call carries `X-Portalai-Timestamp`, `X-Portalai-Webhook-Id`, and `X-Portalai-Signature: v1=<hex>` — the signature is HMAC-SHA256 over `<timestamp>.<webhook-id>.<body>`, bound so a captured request can't be replayed past the 300-second window.",
    example:
      "Register a toolpack, copy the displayed `whsec_...` secret into your server's `TOOLPACK_SIGNING_SECRET` env var, and verify every incoming request before processing — see the Node/Python/Go snippets in `docs/CUSTOM_TOOLPACK_INTEGRATION.md`.",
