...
export const SubscriptionBillingUI: React.FC<SubscriptionBillingUIProps> = ({
  ...
  // Otherwise show the full list to encourage upgrades (#241), collapsing any
  // duplicate `contact` tiers to a single "Enterprise" card (#536 — they all
  // render as the same generic teaser, so more than one is a visual dupe).
  ...
  const displayTiers =
    currentTier?.cta === "contact"
      ? [currentTier]
      : sortTiersForDisplay(collapseContactTiers(tiers));
