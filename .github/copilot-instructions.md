...
One feature = one branch = one PR. Five artifacts land as commits on that branch: ticket (PRD for features / repro + impact for bugs, Issue Type set, sizing recorded), discovery, spec + plan, implementation (one commit per TDD slice), and a **manual smoke checklist** (`docs/<SLUG>.smoke.md`) the user must confirm — merge requires green CI **and** that human confirmation. Each phase has a skill: `/ticket` → `/discovery` → `/spec` → `/plan` → `/smoke`, plus `/epic` for multi-ticket parents. Small tickets take the **condensed path**: `/discovery <N> condensed` writes one combined `docs/<SLUG>.md` instead of the separate docs. **Epics**: `main` auto-deploys to app-dev, so an epic's children branch from and PR into `epic/<slug>` (native sub-issues under an `Epic`-typed parent; keep the epic branch merged up with `main`); one final rebase-preferred PR ships the epic and closes parent + children together. Formatting is enforced (husky pre-commit prettier + CI `format:check`). See CLAUDE.md → "Issue → PR Workflow".
