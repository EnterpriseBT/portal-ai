...
import { BaseModelFactory } from "../../models/base.model.js";
...
function buildBaseModelFactory(idFactory?: IDFactory) {
  return new BaseModelFactory({
    dateFactory,
    ...(idFactory ? { idFactory } : {}),
  });
}
...
describe("UserModelFactory", () => {
  ...
  describe("constructor", () => {
    it("should accept a BaseModelFactory", () => {
      const baseModelFactory = buildBaseModelFactory();
      const factory = new UserModelFactory({ baseModelFactory });
  ...
  describe("create", () => {
    ...
    beforeEach(() => {
      ...
      factory = new UserModelFactory({
        baseModelFactory: buildBaseModelFactory(stubIdFactory),
      });
    });
    ...
    it("should assign the generated id from the underlying BaseModelFactory", () => {
      const model = factory.create("user-1");
    ...
    it("should produce unique ids across multiple calls", () => {
      const defaultFactory = new UserModelFactory({
        baseModelFactory: buildBaseModelFactory(),
      });
      const ids = new Set(
