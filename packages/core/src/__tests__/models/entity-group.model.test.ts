import {
  EntityGroupModel,
  EntityGroupModelFactory,
  EntityGroupSchema,
} from "../../models/entity-group.model.js";
import {
  UUID_REGEX,
  StubIDFactory,
...
const validGroupFields = {
  ...
  name: "Shared People",
  description: "People across all connectors",
  updated: null,
  updatedBy: null,
  deleted: null,
  deletedBy: null,
...
describe("EntityGroupSchema", () => {
  it("should accept valid data with all fields", () => {
    ...
    const result = EntityGroupSchema.safeParse(data);
...
describe("EntityGroupModelFactory", () => {
  describe("constructor", () => {
    it("should accept a CoreModelFactory", () => {
      const coreModelFactory = buildCoreModelFactory();
      const factory = new EntityGroupModelFactory({ coreModelFactory });
      expect(factory).toBeInstanceOf(EntityGroupModelFactory);
    });
  });

  describe("create", () => {
    let factory: EntityGroupModelFactory;
    let stubIdFactory: StubIDFactory;

    beforeEach(() => {
      stubIdFactory = new StubIDFactory("test-id");
      factory = new EntityGroupModelFactory({
        coreModelFactory: buildCoreModelFactory(stubIdFactory),
      });
    });

    it("should return an EntityGroupModel instance", () => {
      const model = factory.create("user-1");
      expect(model).toBeInstanceOf(EntityGroupModel);
    });
    ...
    it("should produce unique ids across multiple calls", () => {
      const defaultFactory = new EntityGroupModelFactory({
    ...
    it("should expose the EntityGroupSchema via the schema getter", () => {
      const model = factory.create("user-1");
    ...
    it("should allow updating domain fields after creation", () => {
      const model = factory.create("user-1");
      model.update(validGroupFields);

      const json = model.toJSON();
      expect(json.organizationId).toBe("org-1");
      expect(json.name).toBe("Shared People");
      expect(json.description).toBe("People across all connectors");
