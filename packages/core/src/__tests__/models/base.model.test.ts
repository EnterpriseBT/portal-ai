...
describe("BaseModelFactory", () => {
  ...
  describe("constructor", () => {
    it("should accept a dateFactory and use the default UUIDv4 id factory", () => {
      const factory = new BaseModelFactory({ dateFactory });
      expect(factory).toBeInstanceOf(BaseModelFactory);
    });
  ...
  describe("create", () => {
    it("should return a BaseModelClass instance", () => {
      const factory = new BaseModelFactory({ dateFactory });
      const model = factory.create("user-1");

      expect(model).toBeInstanceOf(BaseModelClass);
    });
    ...
    it("should use the configured timezone for the timestamp", () => {
      ...
      const factory = new BaseModelFactory({ dateFactory: tokyoDateFactory });
      const model = factory.create("user-1");
      const created = model.toJSON().created!;
...
describe("ModelFactory", () => {
  ...
  const ItemSchema = BaseModelSchema.extend({
    title: z.string(),
  });
  type Item = z.infer<typeof ItemSchema>;

  class ItemModel extends BaseModelClass<Item> {
    get schema() {
      return ItemSchema;
    }
  }
  ...
  function buildFactory(idFactory?: IDFactory) {
    const baseModelFactory = new BaseModelFactory({
      dateFactory,
      ...(idFactory ? { idFactory } : {}),
    });
    return new ItemModelFactory({ baseModelFactory });
  }

  // ── Constructor ─────────────────────────────────────────────────

  describe("constructor", () => {
    it("should store the baseModelFactory", () => {
      const factory = buildFactory();
      expect(factory._baseModelFactory).toBeInstanceOf(BaseModelFactory);
    });
  });
  ...
  describe("create", () => {
    ...
    it("should populate base fields from BaseModelFactory", () => {
      const factory = buildFactory();
      const before = Date.now();
    ...
    it("should use a custom IDFactory passed through BaseModelFactory", () => {
      let counter = 0;
      const seqIdFactory = {
