...
export const FAQ_ENTRIES: FAQEntry[] = [
  ...
  },
  {
    ...
    answer:
      "That pack isn't included in your current plan. Plans decide which tool packs are available, and a station can keep a pack that a later plan change excluded — so instead of removing it, we mark it inactive: the chip stays visible but dimmed, the pack is badged \"Inactive on your plan\" on the Toolpacks page, and it can't be added to another station until the plan includes it. Its tools are absent from portal sessions, which is why the assistant will tell you the capability isn't on your plan rather than pretending it doesn't exist. What's gated is the *assistant's* toolset, not the app: where a screen exists for the same work — creating an entity and its columns, for example — you can still do it yourself regardless of plan. Nothing was deleted, and nothing needs re-attaching: change the plan in Settings → Subscription & Billing and the pack is live again in your next portal session.",
    category: FAQCategory.Analytics,
