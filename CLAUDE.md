...
Two properties worth internalising: the policy is **fail-open on cache availability, fail-closed on cache integrity** (artifacts are HMAC-signed via `remoteCache.signature`; an unreachable cache just runs the work), and **a cache hit can never turn a red check green** — a failing task is never written to the cache at all. One nuance: "fail-closed" describes *verification*. A **missing** signature key does not fail — turbo warns and uploads unsigned — so rule 1 requiring the key in scope is what actually holds that guarantee, not the flag. `apps/web#build` misses on every new commit by design, because the commit SHA reaches its hash through `VITE_APP_SHA`.
...
The API image is built only at deploy (`deploy-dev.yml` / `deploy-prod.yml`), and each deploy **scans before it pushes**: build (load, no push) → **Trivy gate** (fails the deploy on any fixable `CRITICAL`/`HIGH` in the **whole image**, `--ignore-unfixed`) → push with an **SBOM + SLSA provenance** attestation → **cosign keyless** signature on the digest. A failing image never reaches ECR. The runtime image is **minimized so the whole-image gate stays clean**: the runtime stage `apk upgrade`s the OS layer and `rm`s the base image's bundled `npm`/`npx`/`corepack`, and `drizzle-kit` is a **devDependency** (the prod migrate uses `drizzle-orm`'s migrator, not drizzle-kit, so moving it drops the `esbuild` binaries) — the one-off migrate/seed ECS tasks therefore run `node dist/...` directly, not `npm run`. `npm audit` stays the **non-gating** source-tree check (#545); the image gate enforces the shipped artifact. Policy, patch SLA (Critical 7d / High 30d), and the `cosign verify` / SBOM-download runbook live in the durable `docs/SUPPLY_CHAIN.md`. This runs at deploy, not on PRs — a PR does not build the image.
...
- Locks release when the job reaches a terminal status (`completed` / `failed` / `cancelled` — see `TERMINAL_JOB_STATUSES` in `job.model.ts`). No manual unlock paths — but jobs stranded non-terminal by a Redis keyspace loss are reaped to `failed` by the reconciliation sweep (#391, `JobReconciliationService`: BullMQ absence ∧ heartbeat staleness, at boot + every `JOB_STRANDED_SWEEP_INTERVAL_MS`), and `POST /api/jobs/:id/cancel` remains the user's escape hatch.
- **The job-level lock does not cover two passes of the *same* job (#460).** Everything above keys on the job *row*, so it cannot see a BullMQ **stall re-delivery** — which starts a second pass of one job without incrementing `attemptsMade`. Two passes then reap by their own watermarks and the later deletes the earlier's in-flight writes: measured at 34,000 records lost from a 397,960-record layer, on a job reporting `completed` with internally consistent counts. Any operation that reaps by watermark must therefore take `SyncLockService.withInstanceLock(connectorInstanceId, …)` first — a **session-scoped Postgres advisory lock**, chosen over a TTL lease because BullMQ re-delivered precisely *because it guessed wrong* about liveness, and a lease answers that with the same kind of timer. A pass that cannot acquire does no work and reports `superseded` rather than failing. **No reap predicate is an alternative**: "rows this pass did not touch" is correct from each pass's own point of view; the premise that one pass is the only writer is what is wrong.
- **A failed attempt is not a terminal status while BullMQ still holds retry budget (#441).** The worker writes the row and then rethrows so BullMQ can retry; both halves used to run unconditionally, so the row went `failed` with two attempts left — which released the lock on an entity whose worker then wrote ~400K more rows. `statusForFailedAttempt` in `jobs.worker.ts` now writes the non-terminal `pending` when `attemptsMade + 1 < opts.attempts`, and `failed` only when the budget is spent. `UnrecoverableError` is exempt: BullMQ throws it on stall-limit exhaustion, which *voids* the remaining budget rather than consuming an attempt, so it is final on arrival.
- **A job that wrote its data has not failed.** Terminal status reflects whether the *work* succeeded, not whether every best-effort side task did. A sync whose records all landed but whose `er__<id>` mirror cascade failed reports `completed` with `mirrorDegraded: true` on the result (#441/#456) — status describes the data, the result payload describes the degradation. Guard such side tasks in the repository, not per-caller, and keep the degradation on a field something renders: a log line alone is not a surface anyone reads.
- **An attempt that dies without reaching the worker's `catch` still records why — from two complementary hooks (#441, #464).** `worker.on("failed")` catches deaths where a *live* worker receives a failed event, but it runs *inside* a worker process, so a `SIGKILL`ed process cannot run its own, and BullMQ **re-delivers** a stalled job rather than failing it until `maxStalledCount` is exceeded — emitting no failed event at all on the first stall (measured before #464: a worker killed mid-reap left `error` null and `attempts` at 1 across two full executions). That re-delivery case is caught at the *other* end (#464): the resuming execution reads its row before the opening `active` transition and, finding it **still `active`** — a prior execution set it active and died without a terminal/`pending` transition — increments a `lost_executions` column. That start is the one place a re-delivery is knowable, since it neither increments `attempts` nor fires a failed event; the pre-read is **fail-open** (an unreadable row skips the increment rather than sinking a real execution). `attempts` stays the BullMQ attempt count (rendered `N / max` in `JobDetail`); executions get their own field rather than overloading it, and `JobDetail` shows `lost_executions` only when nonzero. The `failed` handler fires for *every* failure, including ones the `catch` already handled, so it must read the row and write only when the status shows the `catch` did not run — otherwise it overwrites a specific error with a vaguer one. Attempt starts also clear the previous attempt's `error` explicitly (`error: null`; Drizzle drops `undefined` from a `SET`, so omitting it keeps the stale text) and record `attempts` 1-based.
...
Tool spend is **server-enforced**, never prompt-enforced (a cost gate the agent can't opt out of — per the standing "safety/confirmation gates get server enforcement, not prompt instructions" rule). Every tool call routes through a build-time wrap in `ToolService.buildAnalyticsTools` (`wrapWithCostGate`), which decorates each tool's `execute` with a two-phase charge (#183): `CostGateService.checkAdmission` runs pre-flight (no charge — a denial returns before the tool runs) and `CostGateService.commitCharge` runs only **after** `execute` resolves successfully, so a thrown call is never charged. Both phases are keyed by the tool's declared `costHint` (`free | metered | expensive` on `ToolCapability`) and the org's `TierPolicy` (#172). Async-job tools (`resultKind: "progress"`) defer the commit to their job processor, charging on job success.
...
**A per-record lookup inside a hot loop degrades quadratically; batch it (#440).** Having the right index is not enough if you probe it one row at a time. `entity_records` has a partial unique index on `(connector_entity_id, source_id)`, yet the sync loop's per-record change-detection read still collapsed once statistics went stale: the planner chose a *different* index and left `source_id` as a residual `Filter`, re-examining every row already written for that entity — **36.015ms per record, growing with each insert**. Batching the same predicate into one `source_id = ANY(…)` of 1000 measured **1.108ms for the whole batch**, because both columns then land in the `Index Cond` and nothing is left to filter. Staleness is structural, not bad luck: autoanalyze fires at `50 + 0.1 × reltuples`, so a 2.4M-row table needs ~236,000 modifications and a single large sync crosses that at most once. The repositories' batch primitives (`findBySourceIds`, `upsertManyBySourceId`, `bulkUpdateSyncedAt`, `wideTable.upsertMany`) all chunk internally — calling them with a one-element array is the bug.

**A soft-deleted table needs a retention policy, and the tombstone side needs its own index (#442).** Soft delete is the default here — every `base.repository` read excludes rows whose `deleted` is set — so tombstones accumulate silently, and the cost is not disk. They count toward `reltuples`, which is what sets the autoanalyze threshold above, so a table that is 77% tombstones (measured: 3.92M of 5.12M rows, 8 GB) raises its own analyze threshold and spends most of every large write on stale statistics. That closes a loop with the paragraph above: tombstones → mis-planned syncs → longer runs → more failures → more tombstones.
...
**Don't assert query plans in the test suites.** The corollary, learned the hard way on the same ticket. A plan guard was written for the above and had to be deleted: the integration fixtures hold ~2,000 rows, where Postgres correctly prefers a sequential scan both before *and* after `ANALYZE`, so the assertion failed permanently while proving nothing about the 400K-row case it was written for. Plan choice is a function of table size and statistics, neither of which a test fixture reproduces. Assert **correctness** in tests (right rows, right counts) and keep plan evidence as a recorded measurement against a production-sized database.
...
- **`--env <name>` is required on every command** (no default) — `local` · `app-dev` · `prod`. `--json` on every command for machine-readable output; the human banner goes to **stderr**, the payload/`{"error":{code,message}}` envelope to **stdout** (parse stdout only).
...
- **Auth:** `cli-env` — AWS-IAM for infra/DB, Auth0 **device-flow** for the app API (`portalai login`; token cached `~/.portalai/credentials.json`, transparent refresh). For `--env local`, DB-touching ops read `DATABASE_URL` (and `AUTH0_*` for `login`) from the CLI's **own package `.env`** — `packages/admin-cli/.env` / `packages/devops-cli/.env`, git-ignored, auto-loaded for local only (#632); copy from the committed `.env.example`. An explicit shell export still wins; app-dev/prod ignore the file and resolve from AWS.
