...
- **An attempt that dies without reaching the worker's `catch` records why *only if some live worker gets a failed event* (#441, narrowed by #464).** `worker.on("failed")` is the hook, but it runs *inside* a worker process — so a `SIGKILL`ed process cannot run its own, and BullMQ **re-delivers** a stalled job rather than failing it until `maxStalledCount` is exceeded, emitting no failed event at all on the first stall. Measured: a worker killed mid-reap left `error` null and `attempts` at 1 across two full executions. Treat a silent `active`→`active` progress reset as a lost execution until #464 lands. It fires for *every* failure, including ones the `catch` already handled, so it must read the row and write only when the status shows the `catch` did not run — otherwise it overwrites a specific error with a vaguer one. Attempt starts also clear the previous attempt's `error` explicitly (`error: null`; Drizzle drops `undefined` from a `SET`, so omitting it keeps the stale text) and record `attempts` 1-based.
