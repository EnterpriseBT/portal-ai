...
> **⚙️ Slices 3–4 — points/lines never-drop (spec amendment 4).** Completes the invariant for the other two geometry types. **Points**: a low-zoom tile that *fits* under the cap now shows individual dots (the per-tile probe), and an over-cap tile aggregates to density bins — never an arbitrary clip. **Lines**: an over-cap tile serves a **hybrid** — the longest `cap` lines drawn raw (a legible skeleton) plus the shorter remainder summarised as density bins — so no line is dropped. Re-smoke: map a **large point layer** (e.g. the ArcGIS points endpoint) and confirm sparse areas show dots while dense areas bin, with nothing blinking as you zoom; map a **large line layer** and confirm dense areas keep the major lines drawn plus a density wash, never going blank. (Verified by integration tests: over-cap points → `aggregated`, over-cap lines → `aggregated` hybrid, a fitting tile → raw.)
