...
2. **Aggregate execution** — new `apps/api/src/services/bulk-aggregate.service.ts` (mirrors `bulk-transform.service.ts`):
   - `explainExpression({ sourceConnectorEntityId, organizationId, expression, sourceFilter })` — assembles `SELECT {expression} FROM "er__{source}" WHERE deleted IS NULL AND organization_id = $1 [AND ({whereSqlFragment})] LIMIT 1` and runs `EXPLAIN`. PG error → throw `BULK_AGGREGATE_EXPRESSION_INVALID` with `details.pgError`.
   - `runAggregate(metadata)` — opens a `READ ONLY` transaction, `SET LOCAL statement_timeout = '120s'`, runs the org-scoped `SELECT {expression}, COUNT(*) AS __records_processed FROM "er__{source}" WHERE deleted IS NULL AND organization_id = $1 [AND (…)]`. Returns `{ result, recordsProcessed }` with `__records_processed` stripped out of `result`. On PG `statement_timeout` error → throw `BULK_AGGREGATE_TIMEOUT`.
