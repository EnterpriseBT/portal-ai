...
### Decision 8 — The units resolver + per-call audit ledger (this ticket's core)

The durable *balance* and its display are #172's (`usage` table, `getBalance`, Settings). What's uniquely this ticket's:

- **`resolveCallCost(tool, input, ctx) → units`** — a per-tool cost function. `web_search` returns `1`; a fan-out tool returns `f(N)` where `N` is input cardinality read via `getMeta(handle).rowCount` *before* execute (the same lookup streaming tools already do). Flat is `weight = 1`; there is no flat-vs-weighted branch. Each tool declares its resolver alongside `costHint` (extends the capability contract — symmetric for built-in and custom, per criterion 3).
...
- **Per-call audit ledger (Postgres) — forensics, not the balance.** #172's `usage` table holds the *aggregate* per-period balance (what billing/the user sees); it does not record *which calls* consumed the units. For chargeback dispute and audit, #169 adds an **append-only** per-call ledger (org, tool, `toolCallId`, units, cost class, tier, outcome, timestamp). This is distinct from #172's balance — aggregate vs. per-call detail — and is separable (the balance alone already caps spend; the ledger is the audit trail). `toolCallId` gives idempotency (a retried SDK call is recorded once).
...
| | D1: build-wrap | D2: DB quota + Redis rate | D5: split ack | D6: error result | D8: units resolver + audit ledger |
|---|---|---|---|---|---|
| Spread to spec | gate prelude + guard test | `UsageService.tryCharge` + rate key | gate ⟂ handshake boundary | result shape + codes | `resolveCallCost` + audit-ledger table |
| New infra | wrap in `buildTools` | 1 rate key (extend `UsageService`) | none (reuse) | 2 ApiCodes | 1 audit-ledger table + per-tool cost fns |
...
| Consumes from #172 (shipped) | — | `resolveTier`, `usage` balance, `periodIdFor` | — | — | tier context for audit rows |
...
5. **Per-call audit ledger (Postgres) for forensics only.** #172's `usage` table is the durable balance + display source; #169 adds an append-only per-call ledger (org, tool, `toolCallId`, units, class, outcome) for chargeback/audit detail. Separable from the balance; `toolCallId` gives idempotency.
...
8. **Built-ins and custom toolpacks share one contract by construction.** Both build through the same `buildAnalyticsTools` → `ai.tool()` wrap and both declare `costHint` + a cost resolver via the same `ToolCapabilitySchema`, so the gate meters them through one code path against one `TierPolicy` — no per-source special-casing. (Caveat: bulk-dispatch fan-out of a *custom* webhook tool builds a separate executor that bypasses the wrap — Open Q7.)
9. **Fail-closed for `expensive` and high-weight `metered`; fail-open only for weight-1 `metered`** when Redis is down (Open Q6) — an unbounded fan-out must not bill uncapped during an outage.
...
5. **~~Increment-then-check atomicity (±1 slop)?~~ [RESOLVED — DB-atomic conditional charge.]** A single call can charge N units, so a read-then-increment race would overshoot by a whole batch and a billing-grade meter can't drift. The quota charge is a **single conditional Postgres UPSERT** (`UsageService.tryCharge`, Decision 2) that increments only if within allocation — atomic, no Redis/DB reconciliation. (Earlier this doc proposed a Redis Lua script; #172 shipping the durable `usage` table made the DB the correct home.)
6. **Failure mode when Redis is down. [Lean firmed for enterprise — split.]** Blanket fail-open lets an unbounded fan-out bill uncapped during an outage. **Lean: fail-closed for `expensive` and high-weight `metered` calls; fail-open only for weight-1 `metered`** (degrades to today's baseline for the cheap case), with a bounded per-instance local fallback bucket softening the edge and a logged warning throughout. *Open sub-question:* the weight threshold that flips fail-open → fail-closed.
7. **Does the gate cover the bulk-dispatch fan-out of a *custom* webhook tool?** `lookupBulkDispatchable` builds a *separate* executor (`ToolService.callWebhook`, `tools.service.ts:300`) for custom tools that bypasses the `buildAnalyticsTools` wrap, so per-row webhook calls aren't individually metered. **Lean: out of scope for v1** — the units resolver charges the *outer* `transform_entity_records` invocation for the fan-out size once (it's a built-in and **is** wrapped), and bulk fan-out is already gated by the ack handshake. If we later meter the per-row webhook calls, the second seam is the dispatcher's injected `toolExecutor` (`bulk-transform-tool.dispatcher.ts`), not the build wrap. Built-in tools dispatched in bulk *are* covered (that path resolves the wrapped `ai.tool()` from `buildAnalyticsTools`).
...
- **Usage analytics / dashboards.** The used/available display is shipped (#172). #169 adds only a per-call **audit** ledger for forensics — no charts/history UI.
- **Metering the per-row webhook fan-out of custom bulk-dispatched tools.** The outer `transform_entity_records` call is charged for the fan-out size once; individual per-row custom-webhook calls are not separately metered (Open Q7).
- **The GIS tools themselves (#84).** This is the surface they consume.

## Next step

**Unblocked — #172 shipped & merged (PR #175).** `TierPolicy`, `resolveTier`, the `usage` balance, and `UsageService.increment`/`getBalance` are live on `main`, so the spec can proceed now. Write `docs/TOOL_COST_GATE.spec.md` (contract: the `resolveCostGate` signature + `resolveCallCost` per-tool cost fn, the deny-result shape, `UsageService.tryCharge` — the atomic conditional-charge extension — + the Redis rate key, the per-call audit-ledger table, the new `ApiCode`s, and the `buildTools` wrap + guard test) and `docs/TOOL_COST_GATE.plan.md` (TDD slices). Likely slicing: (1) `resolveCallCost` per-tool cost fns + the units capability extension (built-in + custom, no gate yet); (2) `UsageService.tryCharge` (atomic conditional charge against the shipped `usage` table) + the Redis rate-limit util + `resolveCostGate` resolver consuming `TierPolicy`, unit + integration tested; (3) the `buildTools` wrap + guard test (asserting **every** built-in *and* custom tool is wrapped — the shared-contract guard), charging `web_search` as the first `metered` consumer; (4) deny-result wiring + the two `ApiCode`s + agent-relay test + the fail-open/fail-closed split; (5) the per-call audit ledger (dual-schema table); (6) docs (`CUSTOM_TOOLPACK_INTEGRATION.md` "costHint now enforced — same contract as built-ins, declare your cost resolver", README/CLAUDE.md cost-control convention). The Settings display needs no work — it already reflects the charged balance (#172). Each slice green and independent; #84's metered tools unblock after slice 3.
