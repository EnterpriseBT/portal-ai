...
3. Over-cap **lines** aggregate too (no arbitrary length-clip). Never-drop coverage by geometry: **points** → nested-grid centroid bins; **lines** → hybrid (longest-N raw + crossed-cell bins); **polygons** → dissolve, always (per the smoke amendment — colorBy per-value, no-colorBy dissolve-all; never centroid-binned).
...
- **no colorBy → area-ranked simplified geometry, no union** (new): the smoke measurements ruled out *any* union approach on the 211k @ ~226-vertex layer — dissolve-all 153s/band, per-cell 132s + 124k rows (both prohibitive vs the 180s budget). `ST_Simplify` alone is ~10s/band and area-ranking is instant, so a plain polygon layer is precomputed as **per-band simplified whole polygons** (one row each, GiST-indexed) and served by clipping the envelope + `ORDER BY ST_Area DESC LIMIT cap` — the **largest polygons in view as real shapes**. Fixes extent (#2) and timeout (#1). Tradeoff: a sub-pixel polygon at extreme low zoom isn't rendered individually (invisible there, appears on zoom-in) — a conscious degradation for pathological layers, analogous to lines' longest-N.
