...
2. **The row cap for inline compute.** Reuse `INLINE_ROWS_THRESHOLD`, or a separate, larger compute cap? Compute data doesn't enter model context (the runtime resolves it), so the ceiling is memory, not tokens. **Lean: a dedicated `COMPUTE_MAX_ROWS` cap, larger than the display threshold**, with a typed `COMPUTE_INPUT_TOO_LARGE` error pointing the agent at SQL pre-aggregation.
