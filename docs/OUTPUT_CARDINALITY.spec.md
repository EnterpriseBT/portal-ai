...
- **Custom-pack subset rule** — `customToolCapabilityError` (`tool-capability.model.ts`) validates `production`: `value` and `rows` allowed; `rows` + `onLarge: "handle"` requires `consumption.mode: "streaming"` (#124 pull-on-read — minting a handle needs a re-readable source). Rejected combinations surface `TOOLPACK_CAPABILITY_INVALID` with a named reason.
