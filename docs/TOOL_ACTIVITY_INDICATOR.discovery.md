...
1. **When does the strip appear?** Now that it is an overlay (Decision 5), appearing and disappearing costs no reflow either way, so this is purely a question of noise. **Lean: only while a step is active** — a pure-text turn is already visibly streaming into the feed, and a pill saying nothing but "thinking" adds occlusion for no information. **Resolved during the smoke walk:** *and* only while the feed is scrolled away from the bottom. Showing the strip alongside the inline indicator put identical text on screen twice, which reads as a duplicate rather than as reassurance — so the two surfaces are mutually exclusive, keyed off the existing `showJumpBottom` signal (`ChatWindow.component.tsx:150-153`). This is the answer the question was actually reaching for: the strip covers the case the inline indicator *can't*, which is precisely when it isn't visible.
