...
2. The frame is a `srcdoc` iframe with `sandbox="allow-scripts"` (nothing else) and an in-document CSP of `default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'`; D3 v7 (new pinned dependency, `apps/web` only) inlined via `?raw` into a module-constant srcdoc.
