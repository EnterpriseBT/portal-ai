...
Put together: **losing the Redis node strands every in-flight job in `active` forever, and every entity those jobs lock stays locked with no supported way to release it.** That is not a cache incident, it is a data-availability incident requiring direct database intervention. It is true in dev today; prod is where it becomes a customer-visible outage.
...
1. ~~What happens to a job that was `active` when Redis is lost?~~ **Answered from the code — see Decision 5.** They are stranded in `active` permanently, and the entities they lock stay locked with no supported unlock. The mitigation here is a `ReplicationGroup` with failover and snapshots; the reconciliation gap is a **separate bug ticket to file**, and it applies to app-dev today.
