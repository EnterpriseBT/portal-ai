...
1. **`apps/site`'s build hash ignores its environment.** Framework inference detects `framework: astro`, which auto-hashes `PUBLIC_*` — but the site reads plain `process.env.SITE_URL` (`apps/site/astro.config.mjs:15`) and `SITE_CONFIG_URL` (`apps/site/src/lib/site-config.ts`), neither `PUBLIC_*`-prefixed. Two dry runs differing only in those values produce the **identical hash `0e93daf1a79163a4`**. With a shared cache, `deploy-site-prod` restores `deploy-site-dev`'s artifact — the production marketing site shipping dev canonical URLs and dev-priced tiers baked into static HTML.
...
6. Mark `apps/site#build` **uncacheable** via a package-level `apps/site/turbo.json`, because its output depends on a live price fetch.
