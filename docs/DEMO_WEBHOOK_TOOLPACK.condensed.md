...
## Decision — auth: HMAC allow-list

The Lambda verifies the `X-Portalai-*` signature (mirroring `mock-toolpack-server.ts`) against **an allow-list of signing secrets** in `PORTALAI_SIGNING_SECRETS` (comma-separated). Because each env's registration generates its own `whsec_…`, one shared endpoint must accept several — the handler tries each with a timing-safe compare. Provisioned after registration via `portalops vars set DEMO_TOOLPACK_SIGNING_SECRETS "<local>,<appdev>,<prod>"` → injected as the Lambda env var. When the list is **empty/unset** the handler returns 401 `SIGNATURE_MISSING` for signed-contract calls (fail-closed) — it never silently accepts unsigned traffic in the deployed state (the mock's SKIPPED mode is dev-only convenience, not copied here).
...
3. **Docs.** Update `docs/DEMO_ORG.runbook.md` prompt 11 with the real tool call + deterministic expected output; link the reference impl from `docs/CUSTOM_TOOLPACK_INTEGRATION.md`; document the registration payload (name, endpoints, tools) so #509 registers it non-interactively.
...
2. `GET /schema` on the built handler (via a local invoke / `mock`-style run) returns the two-tool catalog; `POST /runtime` with a signed request returns the deterministic answer; unsigned → 401. (**Local/agent.**)
