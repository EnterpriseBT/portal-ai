...
4. **A 404 on the first Sheets read after a pick is retryable.** Discovery's passing run differed from the failing one in two ways, so non-instant grant propagation is not excluded.
...
| Grant propagation is not instant | 404 on the first read after a pick | The single bounded retry above |
