...
- [ ] **RED** — Add test: `normalizeMany loads mappings once and normalizes all items`
...
- [ ] **RED** — Add test: `cacheBatchInsert inserts multiple rows in a single AlaSQL call`
...
- [ ] **RED** — Add test: `cacheBatchInsert is a no-op when rows array is empty`
  - Call `applyRecordInsertMany(stationId, entityKey, [])`
  - Verify no AlaSQL calls made (or no error thrown)
- [ ] **RED** — Add test: `cacheBatchUpsert replaces existing rows by ID`
...
- [ ] **GREEN** — Implement public batch methods (all delegate to primitives)
...
- [ ] **REFACTOR** — Ensure single-item `apply*` methods delegate to batch methods with a one-element array (DRY, optional)

### 1.3 `MutationResultContentBlockSchema` extension

File: `packages/core/src/contracts/portal.contract.ts`

- [ ] **RED** — Add test in `packages/core` (or inline assertion): schema accepts `{ type: "mutation-result", operation: "created", entity: "record", count: 5, items: [{ entityId: "a" }] }` without `entityId` at the top level
- [ ] **RED** — Add test: schema still accepts the old shape `{ type: "mutation-result", operation: "created", entity: "record", entityId: "a" }` (backward compat)
- [ ] **GREEN** — Change `entityId` to `.optional()`, add `count: z.number().int().optional()`, add `items: z.array(z.object({ entityId: z.string(), summary: z.record(z.string(), z.unknown()).optional() })).optional()`
- [ ] **REFACTOR** — Run `npm run type-check` to verify no downstream breakage from `entityId` becoming optional
...
- [ ] **RED** — Add test: `single-item regression — { items: [single] } produces same result as before`
...
- [ ] **RED** — Add test: `bulk create — 3 items persisted in single createMany call`
...
- [ ] **RED** — Add test: `bulk create — mixed connectorEntityIds groups correctly`
...
- [ ] **RED** — Add test: `validation failure — scope check fails, nothing written`
...
- [ ] **RED** — Add test: `auto-generates sourceId per item when omitted`
  - Call with 2 items without `sourceId`
  - Assert each created model has a unique UUID `sourceId`
- [ ] **GREEN** — Rewrite `execute` handler
...
- [ ] **REFACTOR** — Extract `groupBy` helper if not already available; verify existing single-item tests still pass
...
- [ ] **RED** — Add test: `single-item regression — { items: [single] } behaves as before`
  - Assert `success: true`, `count: 1`
  - Assert `normalizeMany` called, `updateMany` called with one payload
- [ ] **RED** — Add test: `bulk update — 3 items updated in single transaction`
...
- [ ] **RED** — Add test: `validation failure — record not found for one item, nothing written`
  - Mock `findById` to return null for second item
  - Assert `success: false`, `failures` includes index 1
  - Assert `updateMany` not called
- [ ] **RED** — Add test: `validation failure — record belongs to wrong entity`
  - Mock `findById` to return record with different `connectorEntityId`
  - Assert failure for that item
- [ ] **GREEN** — Rewrite `execute` handler with items wrapper and three-phase pattern
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk delete — 3 items soft-deleted via softDeleteMany`
  - Assert `softDeleteMany` called with 3 IDs
  - Assert scope checks run once per unique entity
- [ ] **RED** — Add test: `validation failure — record not found, nothing deleted`
- [ ] **GREEN** — Rewrite `execute` handler
- [ ] **REFACTOR** — Verify existing tests pass
...
- [ ] **RED** — Add test: `bulk create — 3 new items upserted`
  - Assert `upsertByKey` called 3 times within transaction
  - Assert response `count: 3`, `created: 3`, `reused: 0`
- [ ] **RED** — Add test: `reuse — existing definition with matching key+type is not upserted`
...
- [ ] **RED** — Add test: `reuse with type mismatch — existing key but different type triggers upsert`
...
- [ ] **RED** — Add test: `within-batch dedup — duplicate keys collapsed, last occurrence wins`
...
- [ ] **RED** — Add test: `mixed reuse and create in one batch`
...
- [ ] **REFACTOR** — Ensure `AnalyticsService.applyColumnDefinitionInsertMany` only includes actually-written items (not reused ones)
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk update — 3 items updated in transaction`
  - Mock `findById` to return matching definitions
  - Assert `update` called 3 times with correct data
- [ ] **RED** — Add test: `validation failure — column definition not found`
  - Mock `findById` to return null for second item
  - Assert `success: false`, nothing updated
- [ ] **RED** — Add test: `validation failure — wrong organization`
  - Mock `findById` to return def with different `organizationId`
  - Assert failure
- [ ] **GREEN** — Rewrite `execute` handler with items wrapper
- [ ] **REFACTOR** — Verify existing tests pass
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk delete — 3 items soft-deleted in transaction`
  - Assert `validateDelete` called for each item
  - Assert `softDelete` called 3 times
- [ ] **RED** — Add test: `validation failure — field mappings reference one definition, nothing deleted`
...
- [ ] **RED** — Add test: `bulk create — 3 mappings upserted in transaction`
  - Assert scope checks run once per unique entity
  - Assert all 3 column definitions verified
  - Assert `upsertByEntityAndNormalizedKey` called 3 times
- [ ] **RED** — Add test: `validation failure — column definition not found for one item`
  - Mock `findById` to return null for one `columnDefinitionId`
  - Assert `success: false`, nothing written
- [ ] **RED** — Add test: `validation failure — column definition belongs to different org`
  - Assert failure
- [ ] **RED** — Add test: `bulk create — batch column definition lookup`
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk update — 3 mappings updated`
  - Assert scope checks grouped by `connectorEntityId`
  - Assert `update` called 3 times
- [ ] **RED** — Add test: `validation failure — mapping not found`
- [ ] **RED** — Add test: `validation failure — wrong organization`
- [ ] **GREEN** — Rewrite `execute` handler
- [ ] **REFACTOR** — Verify existing tests pass
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk delete — 3 mappings deleted sequentially`
  - Assert `validateDelete` called for all 3 before any `executeDelete`
  - Assert `executeDelete` called 3 times
- [ ] **RED** — Add test: `validation failure — one mapping fails validateDelete, nothing deleted`
...
- [ ] **RED** — Add test: `bulk create — 3 entities created in transaction`
...
- [ ] **RED** — Add test: `bulk create — mixed connectorInstanceIds`
  - 2 items for "ci-1", 1 item for "ci-2"
  - Assert instance lookup called twice
- [ ] **RED** — Add test: `validation failure — instance not attached to station`
  - Assert `success: false`, nothing written
- [ ] **RED** — Add test: `validation failure — write capability disabled`
  - Assert failure
- [ ] **GREEN** — Rewrite `execute` handler
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk update — 3 entities updated`
  - Assert scope checks grouped
  - Assert `update` called 3 times
- [ ] **RED** — Add test: `validation failure — entity not found`
- [ ] **GREEN** — Rewrite `execute` handler
- [ ] **REFACTOR** — Verify existing tests pass
...
- [ ] **RED** — Add test: `single-item regression`
- [ ] **RED** — Add test: `bulk delete — 3 entities deleted sequentially`
  - Assert `validateDelete` called for all 3 before any `executeDelete`
  - Assert `executeDelete` called 3 times
  - Assert `applyEntityDeleteMany` called once with all 3 entity IDs/keys
- [ ] **RED** — Add test: `validation failure — one entity fails validateDelete, nothing deleted`
  - Assert `executeDelete` not called for any item
- [ ] **GREEN** — Rewrite `execute` handler
...
- [ ] **RED** — Add test: `renders summary text in parentheses for bulk result`
  - Pass `{ ..., count: 5, summary: { entityLabel: "Customers" } }`
  - Assert text contains "(entityLabel: Customers)"
- [ ] **RED** — Add test: `backward compat — old shape with entityId still renders`
...
- [ ] **GREEN** — Update component
  - Derive `isBulk = (content.count ?? 0) > 1`
  - Bulk: render `{count} {entity}s`
  - Single: render `{entity}` (unchanged)
- [ ] **REFACTOR** — Verify no changes needed in `PortalSession.component.tsx` (the `"mutation-result"` type check is unchanged)
...
- [ ] Run `npm run type-check` — verify no TypeScript errors across the monorepo
- [ ] Run `npm run lint` — verify no lint errors

### 7.2 Run full test suite

- [ ] Run `npm run test` from repo root — all existing + new tests pass
...
- [ ] Verify all 12 tool `description` strings mention "1–100 items" bulk capability
- [ ] Verify all `items` field `.describe()` strings document the range
