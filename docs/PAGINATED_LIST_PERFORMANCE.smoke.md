...
- [ ] `git checkout fix/paginated-list-performance && git pull --ff-only`
- [ ] `npm install && npm run build --workspace=packages/core` — `pagination.contract.ts` gained the cursor codec and `entity-record.model.ts` the narrowed list item; the API type-checks and runs against the rebuilt core dist.
- [ ] `cd apps/api && npm run db:migrate && cd ../..` — migration `0081_entity-records-created-sort-index.sql` adds `entity_records_entity_created_id_idx`. Plain `CREATE INDEX` (not `CONCURRENTLY` — drizzle wraps migrations in a transaction), so it briefly blocks writes to `entity_records`; on a ~1.6M-row local DB it took **~6s**.
...
- [ ] **Redis is reachable.** The cached total degrades to a live count without it, so an outage won't break the page — but §8 can't be verified.
...
- [ ] For §7 (writes), the entity's connector instance needs write capability. If the "Add record" button is absent, that's why — skip §7 and say so at sign-off.
...
- [ ] Nothing else to reset — §1–§6 and §8 are read-only.
...
- [ ] Direct API check (substitute a valid bearer token if your dev API requires one):
...
- [ ] Confirm the planner is using the new index rather than sorting the table:
...
- [ ] Cross-check a walk against the database. Pick any page and confirm the ids match:
...
- [ ] In Network, open the `records?...` response. Each entry in `payload.records` has `normalizedData`, `isValid`, `sourceId` — and **no `data` key**.
- [ ] Response size for a 10-row page is a few KB, not tens of KB.
...
- [ ] Copy a `cursor=` value from a Network request on page 3.
- [ ] Re-issue it under a **different sort**:
...
- [ ] Re-issue a deliberately corrupted cursor (`cursor=not-a-cursor`): HTTP **200**, first page again.
...
- [ ] With Network open, page forward several times and watch the `records?...` request timings. They stay flat — no page pays a multi-second count.
- [ ] Confirm the cache is actually being written:
...
- [ ] Change a filter (or type a search term) and page again — a **new** fingerprint key appears, and the total matches the filtered set.
