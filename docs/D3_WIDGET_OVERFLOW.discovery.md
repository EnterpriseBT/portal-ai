...
> **Amended after the first smoke walk (2026-07-27).** Two things this document got wrong, both corrected in `docs/D3_WIDGET_OVERFLOW.spec.md`: (1) measuring the painted extent is *necessary but not sufficient* — escaped marks are clipped a second time by the body box, so the frame must also grow `#root` to the measurement; (2) negative-coordinate content, recorded below as an out-of-scope limitation, is the **common** case (rotated tick labels hang down-left), so it is handled here rather than deferred.
