...
- [ ] **Reprojection (wkid 102100 → 4326).** The source is web-mercator (`wkid: 102100`). Confirm the stored coordinates are **degrees**, not mercator meters: ask the agent "what's the centroid of the first parcel?" (`ST_AsText(ST_Centroid(...))`) → longitude in [-180,180], latitude in [-90,90], and it lands where the parcel actually is (not millions). *(This is the acceptance for slice-4's SRID reprojection.)*
