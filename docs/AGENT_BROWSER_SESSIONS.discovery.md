...
- **Browser-container topology.** This ticket bakes Chromium into the single `dev` devcontainer image (Decision 4), so `localhost:3000` just works and the captured `storageState` origin matches a human's local session. Running the browser as its **own container/service** (e.g. the official `mcr.microsoft.com/playwright:v<ver>` image, browsers + deps preinstalled, connected via `--cdp-endpoint`) is deliberately **not** decided here. It would keep the browser bytes out of every developer's `dev` image, but it adds cross-container networking (`E2E_BASE_URL=http://dev:3000`), an Auth0 Allowed-Callback-URL + `storageState` **origin mismatch** with a human's `localhost` session, and MCP-over-CDP/SSE wiring instead of stdio-local — and saves **no CPU** (a headless browser costs the same wherever it runs; ~0 idle). The split is better motivated for the **deferred CI tier** (services are already separate there, isolation is a feature, the official image is the natural base) than for local dev. If `dev`-image bloat bites before then, the targeted fix is a docker-compose **`profile`-gated** sibling service (opt-in, out of the default `up`). Recorded in the follow-up's scope.
