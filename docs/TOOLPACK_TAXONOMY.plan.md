...
- **E1b — escalation trigger + cost-ack wiring:** decide the trigger first (spec "Open decision — the escalation trigger"; **lean: `EXPLAIN`-predictive + 30s backstop**, *not* reactive-30s — avoid the wasted attempt + double execution). Then `sql_query` gains `acknowledgeCost` + the trigger; on escalation it `recordRejection`s (cost-ack), and on retry-with-ack enqueues E1a's job + returns the handle envelope. Generalizes the existing cost-ack reject→ack→retry flow into the escalation mechanism.
