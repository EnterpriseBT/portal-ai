...
**Build-time config** — `src/lib/site-config.ts`: `fetchSiteConfig(): Promise<PublicSiteConfigResponse>` — fetches `SITE_CONFIG_URL`, parses with `PublicSiteConfigResponseSchema` from `@portalai/core/contracts`; **when `SITE_CONFIG_URL` is set, any fetch error, non-200, or parse failure throws** (build fails loudly — the ticket's rule). Called once, threaded to every page (atomic snapshot). When `SITE_CONFIG_URL` is **unset** (local/root builds with no API running), the fetch falls back to a committed `src/lib/site-config.fixture.json` — validated against the same schema, stamped `fixture` in the build meta tag — so root `npm run build` passes offline (an acceptance criterion). **Both deploy callers always set `SITE_CONFIG_URL`**, so a published build can never be a fixture build; the workflow additionally greps the built HTML for the fixture stamp and fails if present (belt and braces). Build env contract: `SITE_URL` and `SITE_APP_URL` (signup CTA target) default to the dev values when unset; `SITE_CONFIG_URL` unset ⇒ fixture mode.
