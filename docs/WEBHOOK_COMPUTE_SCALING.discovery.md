...
2. **Does inbound pull-on-read unblock D (#129)?** The webhook pulling pages over the cursor is the cursor's first real consumer — so the cursor tier likely lands *here*, handle-backed, scoped to what #124 needs. **Lean: yes — build the minimal cursor inside #124** rather than re-open D speculatively.
...
- **The general unbounded cursor as its own feature** (#129) — #124 builds the *minimal* cursor its pull-on-read needs; a fully general cursor stays #129's concern if a broader caller ever appears.
- **Re-deciding the #121 capability model** — consumed as settled; #124 only widens the custom gate and feeds the declared modes.

## Next step

Write `docs/WEBHOOK_COMPUTE_SCALING.spec.md` (the `{ tool, input, records }` POST contract; the signed read-endpoint shape + auth; `produceFromRows`; the `{ resultHandle }` response envelope; the widened gate) and `docs/WEBHOOK_COMPUTE_SCALING.plan.md`. The plan slices: `produceFromRows` → records-in-body (`bounded`) + gate-widen → signed read endpoint + pull-on-read (`streaming`) → outbound `{ resultHandle }` → contract-surface/docs/mock-server — each green-testable on its own, the security-sensitive read endpoint isolated in its own slice with its own adversarial tests.
