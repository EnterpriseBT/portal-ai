...
- [ ] **Auth verifies against the OIDC issuer**: the JWKS/issuer `jwtCheck` now uses is `OIDC_ISSUER` (not Auth0). A token from the configured OIDC issuer validates; an Auth0 token is rejected (401). **DEFERRED (recorded):** full OIDC token federation needs a real customer OIDC issuer, which we don't have locally — it will be walked during the **epic smoke in app-dev with a mock customer organization** (and lands properly with #577). Locally confirmed: the boot uses the OIDC config and unauth requests 401 (§2 above).
