...
- **Route-redirect guard** — an **in-React hook** `useRequirePageView(pageId | pageId[])` + a `guardedComponent(pageId, View)` HOF applied as each gated index route's `component`. It redirects to Dashboard when `!some(canViewPage)` (the Connectors route passes `["connectors","connector_catalog"]`, so a partial grant is never bounced). **Not a TanStack `beforeLoad`**: the auth token is only available through the `useAuth` hook (`api.util.ts`), so a `beforeLoad` (outside React) can't fetch the current-org query. Optimistic during load (no blank on the allowed path), fail-closed once known.
- **`ForbiddenView` is *not* wired** — the model needs no 403 surface: list pages redirect, and an unreadable detail object returns **404** (invisible == absent, slice 2), handled by the existing not-found states. Recorded as an intentional deviation from the discovery's "Denied UX" lean.
