...
**Two source kinds (revised 2026-08-02).** A layer's data arrives either as **inline GeoJSON** — rows already delivered by the sink, for small results — or from a **vector-tile source** pointing at #316's org-scoped `ST_AsMVT` endpoint, for arbitrarily large ones. The tile source is the **spatial analogue of a query handle**: server-held query, client pages by `z/x/y` instead of `offset/limit`, spatial index in place of the row cursor — and unlike a row handle it needs no Redis staging, so a pinned map stays explorable long after an equivalent table's handle would have expired.

Tiles are authenticated requests, which is **why the renderer is direct-mount rather than sandboxed**: a sandboxed program would need a credential passed into model-authored JS. The reserved `program` hatch therefore applies to inline-data maps only. `MapLayerSchema.source` therefore gains a tile variant alongside `geometryColumn` / lat-lng, and the widget maps it to a native MapLibre vector source. This is what makes "large geometry dataset" a server concern rather than a client limit: tiles transfer only the viewport, simplified per zoom, so `MAP_LAYER_FEATURE_CAP` applies to the inline path only. #314 pins the exact discriminant.
