...
5. **How does the wire schema (`ToolpackToolDefinitionSchema`) evolve so custom packs can declare capabilities without breaking already-registered packs?** **Lean: additive optional fields** defaulting to today's behavior (pure, inline) — same posture as `bulkDispatch`'s optional add.
