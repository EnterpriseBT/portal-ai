...
### Decision 8 — Cardinality is a runtime mode, not a tool identity (the `bulk_*` prefix dissolves)

Falls out of D2 + D3. Once the runtime selects inline / handle / job from N (bounded above by declared `consumption`), a `bulk_X` tool sitting beside `X` re-bakes cardinality into tool identity — seam 2, but at the tool level. **A. Keep `bulk_*` as distinct tools** (status quo; the agent picks the scaled variant). **B. Collapse them** — the agent names the *operation* once; the runtime escalates to job mode by N.

**Decided: B** (confirmed 2026-06-16). `bulk_aggregate_records` is `sql_query`/aggregate at job mode; `bulk_transform`/`bulk_apply` are the transform op at job mode; `bulk_delete` is delete at job mode; `bulk_materialize` is materialize at job mode. **Cost-ack (`costHint`) and the entity lock (`locks`) fire as runtime gates on job escalation (D5), not as separate tools**; the job result's `resultKind` is `progress`/`mutation-result`. The pack (`data_query`, `entity_management`) stays the enablement/UI unit. This removes the "give the bulk tools a descriptor home" half-step — #101/#102/#112 ship as the *job mode* of their operations, not as new tools. (Distinct delivery semantics — async 202 + progress + lock — are real, but are runtime behaviors of the operation, not a reason for a second tool name.)
...
## Custom toolpacks under the new taxonomy

**Flow + UX preserved (hard constraint).** Registration is unchanged — the user enters schema/runtime/auth URLs in `RegisterToolpackDialog`; the portal fetches `/schema`, validates, caches; packs attach/detach per station. **No new fields in the dialog.**

**The served `/schema` gains capability metadata, author-declared** (same place `name`/`parameterSchema` already come from; clean-cut per Open Q6). A custom tool runs third-party with no backend access, so it may declare only the **pure-consumer subset**:

| Field | Custom tool |
|---|---|
| `pure` | forced **true** |
| `reads` / `writes` (entities) | **rejected** |
| `consumption` | `none` / `bounded(maxRows)` (=#122 records-in-body) / `streaming` (=#124 pull-on-read); **not** `engine-pushdown` |
| `computeShape` | `map` / `reduce` / pure; not backend scan/mutate |
| `resultKind` | declarable (`data-table`/`scalar`/`vega`/`d3`/`geo`) → renderer routing |
| `costHint` | declarable |
| `locks` / `alwaysAvailable` | **rejected** |

`ToolpackRegistrationService.fetchSchema` **validates the subset** — a custom tool claiming `engine-pushdown` / backend `writes` / `locks` fails registration with a clear, named error. Runtime (`buildAnalyticsTools` / `WebhookTool`) reads the declared capabilities: `consumption` → record-source feeding (bounded = #122; streaming = #124), `resultKind` → `resolveDisplayBlock` routing, `costHint` → cost gate, `map`+`bulkDispatch` → per-record dispatch.

**UI/UX — two personas:**
- *User/admin* (registers + attaches in the portal): **workflows unchanged.** Additive, read-only gains: **capability badges** in the metadata modal ("consumes records · returns chart · expensive"), **clearer registration validation errors** (disallowed capability named), and — from D5 per-tool write-gating, mainly for built-in `entity_management` — an optional "*N of M tools need write access*" indicator when a pack is partially active.
- *Author* (writes the server + served schema): now **declares capabilities in `/schema`**; the dialog's reference section + `CUSTOM_TOOLPACK_INTEGRATION.md` document the capability fields + the records/handle consumption contract + `resultKind`.

**Net:** custom packs become first-class in the capability model, constrained to the "pure, no privilege" corner — enforced at registration, surfaced read-only in the UI, register/attach workflows untouched. (Registration-schema expansion is part of #121's foundation slice; the custom runtime wiring + the `streaming` path are #124.)

## D4 spike — reduce-tier expressibility (resolves Q2)

Read all 18 compute-tool implementations against Postgres (aggregates, `corr`/`regr_*`, `percentile_cont`, window frames, recursive CTE). Classifying by **what scales with N** (the O(N) row work), not by whether *every* sub-step is SQL:

- **Push into the engine — 10 (removed as agent-facing tools).** Their entire data-scale work is a SQL aggregate/window: `describe_column` (mean/stddev/quantiles/skew/kurtosis), `correlate` (`corr`; Spearman/Kendall via rank CTE), `detect_outliers` (z-score/IQR/MAD), `aggregate` (literally `GROUP BY`), `trend` (`date_trunc` + `regr_*`), `changepoint` (CUSUM = cumulative-sum window), `decompose` (classical = moving-average windows), `sharpe_ratio`, `max_drawdown` (running-max window), `rolling_returns` (`lag`). The §6 smoke already showed the agent doing these in `sql_query` — confirmed dead weight.
- **Engine-pushdown + O(1) scalar residue — 3 (kept, but unbounded-safe).** The O(N) reduction is a SQL aggregate; only a constant-size, data-independent step isn't: `hypothesis_test` (statistic in SQL; p-value = incomplete-beta/gamma CDF on scalars), `var_cvar` (historical = `percentile_cont`/tail-mean SQL; parametric quantile is O(1)), `regression` (`X'X`/`X'y` accumulate as `sum`-aggregates; the k×k solve is O(1) in N). `consumption: engine-pushdown` → **exact at any N**.
- **Genuine escape hatch — 5 (kept as pure-fn consumers).** O(N) work is not a single SQL aggregate:
  - *Streaming (single-pass stateful → exact at any N):* `forecast` (Holt-Winters recurrence), `technical_indicator` (EMA recurrence + multi-period), `portfolio_metrics` (cumulative-product wealth + downside). `consumption: streaming`.
  - *Bounded (multi-pass optimization → needs the whole set):* `cluster` (k-means), `logistic_regression` (IRLS). `consumption: bounded(maxRows)` + `onOverflow`; the upgrade to exact-unbounded is a streaming variant (mini-batch k-means, SGD logit).

**Verdict (Q2 resolved): 13 of 18 are engine-expressible for their data-scale work (10 pushed + 3 pushdown), 5 are genuine escape hatches, and only 2 (`cluster`, `logistic_regression`) can't be exact-unbounded without an approximate streaming variant.** That last "2" is the concrete residue of the Appendix caveat — far smaller than feared. Engine: reuse the Postgres `sql_query` path (Q4) — no coverage gap forces DuckDB.

## Target tool census (slice-map input)

Reflects the spike above **and Decision 8** (cardinality is a runtime mode — the `bulk_*` prefix dissolves). **No new packs**, pack list + attach/register UX unchanged; what changes is membership (reduce tier 18 → 8 tools; bulk tools dissolve into operation job-modes; #115/#116 drift closed) and the per-tool capability interface.

**The new interface (every tool, builtin + custom).** `ToolpackTool` / `ToolpackToolDefinitionSchema` gains:
```
{ pure, reads[], writes[], consumption{ mode: engine-pushdown|streaming|bounded, maxRows?, onOverflow? },
  computeShape: scan|reduce|map|mutate|visualize|pure, costHint, locks[], resultKind, alwaysAvailable }
```
Notation: `privilege · consumption · shape · resultKind [· flags]`. **bold** = new tool.

### `station_context` (system · alwaysAvailable)
| Tool | Interface |
|---|---|
| `current_time` | pure · none · pure · `scalar` · alwaysAvailable |
| `station_context` | reads · engine-pushdown · scan · `data-table` · alwaysAvailable |

### `data_query` (read · reduce · visualize — one op each; runtime picks cardinality)
| Tool | Interface | Δ |
|---|---|---|
| `sql_query` | reads · engine-pushdown · scan/reduce · `data-table` (producer) | **absorbs** `bulk_aggregate_records` (reduce at job mode, D8) **+ the 10 SQL-pushed reduce tools** |
| `display_entity_records` | reads · engine-pushdown · scan · `data-table` (producer) | descriptor fixed (#116) |
| `resolve_identity` | reads · engine-pushdown · scan · `scalar` | enriched |
| `visualize` → `vega`, `visualize_tree` → `vega-tree` | reads · engine-pushdown (aggregate-before-render) · visualize | D6 |
| **`network_graph`** (D3) | reads · engine-pushdown · visualize · `d3` | NEW (D7; #92-adjacent) |
| **`map_view`** (GIS) | reads · engine-pushdown · visualize · `geo` | NEW (D7; #84) |

### Reduce tier — `statistics` / `regression` / `financial` (same packs, far fewer tools)
| Tool | Pack | Interface |
|---|---|---|
| `cluster` (k-means) | statistics | pure · **bounded**(maxRows)+onOverflow → streaming (mini-batch) · reduce · `data-table`/`vega` · expensive |
| `logistic_regression` (IRLS) | regression | pure · **bounded**+onOverflow → streaming (SGD) · reduce · `data-table` · expensive |
| `forecast` (Holt-Winters) | regression | pure · **streaming** · reduce · `vega`/`data-table` |
| `technical_indicator` (EMA etc.) | financial | pure · **streaming** · reduce · `data-table`/`vega` |
| `portfolio_metrics` | financial | pure · **streaming** · reduce · `scalar`/`data-table` |
| `hypothesis_test` | statistics | pure · **engine-pushdown** (stat in SQL; p-value O(1)) · reduce · `scalar` |
| `var_cvar` | financial | pure · **engine-pushdown** (historical SQL; parametric O(1)) · reduce · `scalar` |
| `regression` (OLS) | regression | pure · **engine-pushdown** (`X'X` aggregates; k×k solve O(1)) · reduce · `data-table` |
| `npv` `irr` `tvm` `xnpv` `xirr` `depreciation` `amortize` `bond_math` | financial | pure · none (inline params) · pure · `scalar`/`data-table` |
| ~~`describe_column` `correlate` `aggregate` `detect_outliers` `trend` `changepoint` `decompose` `sharpe_ratio` `max_drawdown` `rolling_returns`~~ | — | **removed → expressed in `sql_query`** (10 tools) |

### `entity_management` (writes; the job is a *mode*, not a tool — D8)
| Tool | Interface | Δ |
|---|---|---|
| `entity_record_create/update/delete` | writes[entity_records] · bounded(small inline)→job at scale · mutate · `mutation-result`/`progress` · locks[recordIds] | per-tool `writes`/`locks` (D5); **job mode replaces** `bulk_transform_entity_records`/`bulk_apply` |
| `connector_entity_create/update/delete` | writes · mutate · `mutation-result` · locks[connectorInstanceId] | D5 |
| `field_mapping_create/update/delete` | writes · mutate · `mutation-result` · locks | D5 |
| *delete @ job mode* (was `bulk_delete` #101) | writes · engine-pushdown · mutate · `mutation-result`/`progress` · locks · expensive | **mode, not a tool** |
| *transform/apply @ job mode* (was `bulk_transform`/`bulk_apply` #102) | writes · streaming · map · `progress` · locks · expensive · bulkDispatch | **mode, not a tool** |
| *materialize @ job mode* (was `bulk_materialize` #112) | writes (produces entity) · engine-pushdown · mutate · `progress` · expensive | **mode, not a tool** |

### `web_search` (server-side)
| Tool | Interface |
|---|---|
| `web_search` | reads (external) · none · scan · `data-table` |

### Custom-webhook packs (per registered pack)
Each served tool, pure-consumer subset (validated at registration): `pure` (forced) · `none`/`bounded`/`streaming` · `map`/`reduce`/pure · `resultKind` declarable · costHint declarable. `bounded` = #122 records-in-body; `streaming` = #124 pull-on-read. No `writes`/`engine-pushdown`/`locks`/`alwaysAvailable`.

**Net deltas:** reduce tier **18 → 8** (10 pushed into `sql_query`; 3 become engine-pushdown; 5 stay as escape hatches — only `cluster`/`logistic_regression` are `bounded`). The `bulk_*` tools **dissolve into operation job-modes** (D8) — `bulk_aggregate`/`bulk_transform`/`bulk_delete`/`bulk_apply`/`bulk_materialize` are no longer tools; #101/#102/#112 ship as job modes. 2 new viz tools (`network_graph`, `map_view`). Every surviving tool gains the capability interface. **Pack count + attach/register UX unchanged.**
...
2. **How much of the 18-tool reduce tier is SQL/window-expressible (Decision 4 spike)?** **Resolved (spike, 2026-06-16): 13 of 18.** 10 push fully into `sql_query` (removed as tools); 3 (`hypothesis_test`, `var_cvar`, `regression`) become `engine-pushdown` (O(N) reduction in SQL, O(1) residue); 5 stay escape hatches (`forecast`/`technical_indicator`/`portfolio_metrics` stream exact at any N; `cluster`/`logistic_regression` are `bounded`+onOverflow with mini-batch/SGD upgrade paths). Only 2 tools can't be exact-unbounded without an approximate variant. See "D4 spike" section.
