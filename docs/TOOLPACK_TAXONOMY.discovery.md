...
**Lean: B.** The load-bearing thing is *what a tool does*, declared once; UI grouping, enablement, and enforcement become projections. `costHint` already lives on metadata — extend the pattern to `locks`/`alwaysAvailable`/`resultKind`.
...
|  | D1 capability metadata | D2 handle currency | D3 runtime mode | D4 shrink reduce | D5 declarative enforce | D6 viz-consumer |
|---|---|---|---|---|---|---|
| Spread to spec | Yes — the metadata schema | Yes — producer/transformer/consumer | Yes — mode-select rule | Spike first, then spec | Yes — per-enforcement | Yes — resultKind + agg-before-render |
| Dissolves a seam | 1 + 3 | 1 | 2 | (tier size) | 3 | 4 (+ #120) |
| Ships as | foundation slice | foundation slice | slice | spike → slices | slice | slice |
...
6. **Visualization consumes handles + aggregates before render**; display routing reads a declared `resultKind` (fixes #120). #92/#84 are downstream consumers.
...
1. **Does "toolpack" survive as an enforcement unit, or collapse to UI/discovery grouping?** **Lean: collapse** — enforcement + agent-surface move to capability metadata + station config; the pack name is just a label and a UI bucket.
...
5. **How does the wire schema (`ToolpackToolDefinitionSchema`) evolve for custom packs to declare capabilities?** **Lean: clean cut — no back-compat shim.** There are **no registered toolpacks in production yet** (confirmed 2026-06-16), so the capability fields can be first-class (required where it makes sense) rather than additive-optional-for-compat; no dual-write, no migration of existing packs, no destructive-migration risk. Builtins still default sensibly in code. Aligns with the project's no-production-data posture and the no-compat-aliases convention — and removes a migration/back-compat slice from the slice map.
