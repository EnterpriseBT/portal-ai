...
The taxonomy spawns **no new packs** — "bulk" is a `consumption`/`costHint` value, not a category (a bulk pack would re-make seam 2). The pack list + attach/register UX stay; what changes is *membership* (reduce-tier shrinks; the homeless bulk tools get descriptor homes; the registry drift from #115/#116 is closed) and the **per-tool capability interface** every tool now declares.
...
```
Notation below: `privilege · consumption · shape · resultKind [· flags]`. `†` = membership decided by the D4 spike (Open Q2). **bold** = new tool or new descriptor home.
...
### `data_query` (producers + visualize)
| Tool | Interface | Δ |
|---|---|---|
| `sql_query` | reads · engine-pushdown · scan/reduce · `data-table` (producer) | enriched |
...
| `visualize` | reads · engine-pushdown (aggregate-before-render) · visualize · `vega` | D6 |
| `visualize_tree` | reads · engine-pushdown · visualize · `vega-tree` | D6 |
| `bulk_aggregate_records` | reads · engine-pushdown · reduce · `data-table`/`scalar` · costHint=expensive | descriptor home (was drift) |
...
### `statistics` — shrinks to the non-SQL escape hatch
| Tool | Interface | Δ |
|---|---|---|
| `cluster` (k-means) | pure · bounded(maxRows)+onOverflow=sample → **streaming** (mini-batch) · reduce · `data-table`/`vega` · costHint=expensive | kept; #120 routing fixed by `resultKind` |
| `hypothesis_test` | pure · bounded · reduce · `scalar` | kept† |
| `describe_column` | → SQL engine aggregate | removed† |
| `correlate` | → SQL `corr()`/window | removed† |
| `aggregate` | → SQL `GROUP BY` | removed† |
| `detect_outliers` | → SQL quantile/z-score | removed† (also fixes #120) |

### `regression` — shrinks to the iterative escape hatch
| Tool | Interface | Δ |
|---|---|---|
| `logistic_regression` (IRLS) | pure · bounded+onOverflow · reduce · `data-table` · costHint=expensive | kept |
| `forecast` (Holt-Winters) | pure · bounded+onOverflow=error → streaming · reduce · `vega`/`data-table` | kept |
| `decompose` | pure · bounded · reduce · `vega` | kept |
| `changepoint` | pure · bounded · reduce · `data-table`/`vega` | kept |
| `regression` (OLS) | pure · bounded · reduce · `data-table` | kept† (simple OLS may push to `regr_*`) |
| `trend` | → SQL `regr_slope`/`regr_intercept` | removed† |

### `financial` — 8 pure-math kept; 6 compute kept (non-SQL)
| Tool | Interface |
|---|---|
| `npv` `irr` `tvm` `xnpv` `xirr` `depreciation` `amortize` `bond_math` | pure · **none** (inline numeric params, not handle-consuming) · pure · `scalar`/`data-table` |
| `sharpe_ratio` `max_drawdown` `rolling_returns` `var_cvar` `portfolio_metrics` `technical_indicator` | pure · bounded/streaming · reduce · `scalar`/`data-table`/`vega` (some window-expressible — push†) |

### `entity_management` — writes; bulk-write family folds in
| Tool | Interface | Δ |
|---|---|---|
| `entity_record_create/update/delete` | writes[entity_records] · bounded(small inline) · mutate · `mutation-result` · locks[recordIds] | per-tool `writes`/`locks` (D5) |
...
| `bulk_transform` / `bulk_transform_entity_records` | writes · streaming · map · progress/`mutation-result` · locks · costHint=expensive · bulkDispatch | descriptor home (was drift) |
| **`bulk_delete`** (#101) | writes · engine-pushdown/streaming · mutate · `mutation-result` · locks · costHint=expensive | NEW |
| **`bulk_apply`** (#102) | writes · streaming · map · `mutation-result` · locks · costHint=expensive · bulkDispatch | NEW |
| **`bulk_materialize`** (#112) | writes (produces entity) · engine-pushdown · mutate · `mutation-result` · costHint=expensive | NEW |
...
**Net deltas:** ~6–8 reduce tools collapse into the engine (spike-gated); 5 new tools (`network_graph`, `map_view`, `bulk_delete`, `bulk_apply`, `bulk_materialize`); 3 drift tools (`bulk_aggregate_records`, `bulk_transform*`, `display_entity_records`) get formal descriptor homes; every surviving tool gains the capability interface. Pack count and attach/register UX unchanged.
