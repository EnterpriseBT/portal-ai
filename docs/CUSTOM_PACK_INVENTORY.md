...
- **Files.** Edit `apps/api/src/tools/station-context.tool.ts` (call the resolver; add `custom` to the `toolPacks` section and to the tool's own description of that section) and `apps/api/src/config/swagger.config.ts` **if** the station-context response shape is registered there — verify at implementation time; `:1266-1278` carries a `toolPacks` shape whose owner needs confirming.
