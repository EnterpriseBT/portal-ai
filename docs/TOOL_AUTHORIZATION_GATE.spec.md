...
- **Bulk correctness** is a class-level admin gate (no per-row predicate to get wrong): a member is denied a whole-entity scan, an admin is allowed. The O(1) + per-object tests are the detector. Member-scoped bulk is **not** deferred processor work — it arrives with curated views (#599): a bulk job that targets a permission-scoped **view** (an RBAC object) is authorized by a single view-object check (the gate's `single` mode already handles it) and the view bounds the rows. The class-level gate is the correct interim behavior and is forward-compatible — the bulk descriptor gains a `viewId` target and flips to `single` when view-targeting lands; no gate change.
