...
5. **Reduce tier 18 → 8** (D4 spike, Q2 resolved). 10 tools are removed (expressed in `sql_query`); `hypothesis_test`/`var_cvar`/`regression` become `engine-pushdown`; `forecast`/`portfolio_metrics`/`cluster`/`logistic_regression` are `streaming` reduces (`cluster` = mini-batch k-means, `logistic_regression` = AdaGrad SGD — #153; exact in-memory for small N, online fold past it); `technical_indicator` is a `streaming` per-row **map** (a large source folds into a cursor-backed transform handle, #159). With #153 the whole reduce tier is exact/bounded-error-unbounded — no tool hits a hard wall.
...
| **streaming reduce** (4) | `forecast` `portfolio_metrics` `cluster` `logistic_regression` | `pure:true` · `streaming` · reduce. `cluster` (mini-batch k-means) + `logistic_regression` (AdaGrad SGD) are exact in-memory for small N and fold online past it (#153), `costHint:expensive`. |
| **streaming map** (1) | `technical_indicator` | `pure:true` · `streaming` · **map** · `resultKind:data-table` — a per-row N→N series, not a reduce. Small input returns the series inline; a large query handle folds in one ordered pass into a cursor-backed **transform handle** (#159), so it scales to any N. |
...
- [ ] Reduce tier is 8 tools; the 10 removed are expressed in `sql_query` (smoke-verified per pack); every reduce-tier tool is exact/bounded-error at any N (`cluster`/`logistic_regression` fold online past the in-memory threshold, #153).
