...
> **Smoke amendment 2 — unified area-ranked (supersedes the split below).** A second re-smoke established that the colorBy *union* (merged-region choropleth) and the no-colorBy area-ranked path should **not** be two flavors: the aggregation *decision* must be identical for both, differing only in the aggregate *representation*. And the count-driven "raw when few" behavior a user expects can't come from a per-tile pipeline probe — that re-introduces exactly the per-tile pipeline scan the precompute exists to avoid (it recreated the tile storm on the 22k layer). The resolution: **one area-ranked path for both flavors.** The precompute stores **one row per source polygon per band**, simplified to the band tolerance, tagged with its colorBy value (or the `"__all__"` sentinel) — **no union, no subdivide, no cardinality gate**. The serve **always** `ORDER BY ST_Area(mdg.geom) DESC LIMIT cap`, emitting the value only for a colorBy layer. This is inherently count-driven and pipeline-free: a tile under the cap shows **every** polygon in view (the `LIMIT` is a no-op) at the band's detail (fine bands ≈ raw), one over it shows the largest by area — never-dropping the visible ones. `DISSOLVE_CARDINALITY_CEILING` and the `over-cardinality`/`no-colorby` skip reasons are retired (a high-category choropleth is no longer a storage problem when it stores one row per polygon). Slice 6's derive-from-finest union continuity is moot (there is no union). `AGG_TILE_VERSION → 5`.
