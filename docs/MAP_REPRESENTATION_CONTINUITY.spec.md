...
> **Amendment 4 — slices 3–4 as built (points probe + line hybrid).** Two refinements to the pre-implementation plan above:
> - **`truncated` is NOT fully retired.** Points/lines never clip now (under cap → raw, over → aggregate/hybrid), and the polygon dissolve never clips — but a polygon **message block** (no precompute) still serves the raw path and clips over cap (the documented, out-of-scope message-map limitation). So `TileQueryResult.truncated` / `truncatedCap` / the header / the "Partial at this zoom" web notice are **kept**: they now fire in exactly one place, and it's the right one. `TileMode` keeps `"dissolve"` too (returned by the polygon path). No `mode`-field refactor — `truncated`/`aggregated` stay.
> - **Line hybrid = longest-`cap` raw skeleton + centroid-density bins of the *remainder*.** `buildLineHybridTileSql` ranks by `ST_Length` DESC, draws `rn ≤ cap` raw (no `_agg`), and bins `rn > cap` by `ST_SnapToGrid(ST_Centroid(...))` with `_count`/`_agg:1` on the same nested grid as points. A centroid (not cross-every-cell) bin is sufficient because the binned remainder is the *shortest* lines — the long lines that would span many cells are in the raw skeleton. The probe is `buildRawTileSql` at `LIMIT cap+1` inside the session-view txn (cheap for points/lines: indexed envelope filter, no per-row simplify — the polygon tile-storm risk does not apply), doubling as the raw serve when the tile fits. The web `-agg` fill + `_agg` split now also covers tiled **line** layers (density ramp; a colorBy line's bins still use density, since the hybrid bins carry no value).

> **Smoke amendment 3 — count-driven merged coverage (the never-drop fix; supersedes amendment 2's serve).** A third re-smoke found that "area-ranked, keep the largest `cap`" **violates the never-drop invariant** — the ticket's core requirement. Measured on the live CA-block-groups pin (22k, complete precompute): a z5 tile holds **21,957 polygons**, over the 10k cap, so the serve kept the largest 10k and **dropped ~12k of the smallest** (dense urban) — whole swathes vanished at low zoom and reappeared on zoom-in. Area-ranking *drops*; it does not *aggregate*. The fix: the precompute stores **two representations per band**, distinguished by a new `merged boolean` column: **individuals** (`merged=false`, one row per source polygon, simplified, tagged with value — for the under-cap serve) **and a merged coverage** (`merged=true`, one `ST_Union` per value per band, derive-from-finest + `ST_Subdivide`d — for the over-cap serve). `runDissolveTile` is **count-driven**: it counts the individual rows in the envelope; **≤ cap → serve individuals** (every polygon as itself), **> cap → serve the merged coverage** (every polygon represented as part of the dissolve — nothing dropped). As a tile falls under the cap on zoom-in, individuals emerge from the merged shape — continuous, never blinking. A merged tile reports `aggregated=true` (an aggregate overview notice); an individuals tile `aggregated=false`. Still pipeline-free (both representations read from the GiST-indexed table). On a layer too large to union in budget (211k), the merged pass fails degraded — individuals still serve and an over-cap tile falls back to the area-ranked drop rather than blanking (documented; the huge-layer follow-up bounds it). `AGG_TILE_VERSION → 6`; migration `0091` adds `merged` + folds it into the lookup index.

> **Smoke amendment 2 — unified area-ranked (superseded by amendment 3 for the serve; the two-representation storage keeps its individuals path).** Established that colorBy and no-colorBy must share one aggregation *decision*, differing only in representation, and that a per-tile pipeline probe is the wrong count source (it re-runs the pipeline per tile — the tile storm). The precompute stores per-polygon individuals tagged with the colorBy value (or `"__all__"`); `DISSOLVE_CARDINALITY_CEILING` and the `over-cardinality`/`no-colorby` skip reasons are retired. (Amendment 3 adds the merged coverage back *alongside* these individuals, count-selected — not the old always-merged choropleth.)

_Original split (retained for history; superseded by amendment 2):_

- **colorBy → per-value dissolve** (existing, made continuous in slice 6): `ST_Union` per colorBy value per band, `ST_Subdivide`d into pieces. Cardinality-gated by `DISSOLVE_CARDINALITY_CEILING`.
- **no-colorBy → area-ranked simplified geometry**: **no union.** Per band, `ST_SimplifyPreserveTopology(geom, tol(band))` each polygon and store **one row per polygon** under a sentinel `column_name = value = "__all__"`, `feature_count = 1`.

`hasDissolvePrecompute` is unchanged (existence by `(pin, column_name, band)`). `runDissolveTile` area-ranks **both** flavors (`ORDER BY ST_Area DESC LIMIT cap`), emitting the stored `value` as the colorBy property only when the column is not the `"__all__"` sentinel. `defaultRunTileQuery` passes the cap through. `DissolvePrecomputeService.isDissolvable`/`enqueueForPin` enqueue **every** polygon layer (not only colorBy).
...
- [ ] **Polygons render as real geometry at every zoom — never centroid-bin squares.** Unified area-ranked (amendment 2): a tile under the cap shows every polygon in view as individual real shapes (colored by value for a colorBy layer); over the cap, the largest-by-area. The aggregation *decision* is identical for colorBy and no-colorBy.
