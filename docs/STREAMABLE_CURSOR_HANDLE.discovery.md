...
1. **Concrete consumers to anchor this.** Two, now: (a) the builtin streaming reduces (`forecast`/`technical_indicator`/`portfolio_metrics`) over >100k ordered points, and (b) **#92 trace-based pins** — a pin re-executes its SQL/tool pipeline on view, so a pin over a >100k pipeline streams server-side through this cursor. Plus (c) #124's remote pull-on-read. **The substrate has real callers; A's re-execution primitive is shared across all three.** (A product gut-check on how common >100k pin/forecast pipelines are still sharpens priority, but the substrate is no longer a one-consumer bet.)
