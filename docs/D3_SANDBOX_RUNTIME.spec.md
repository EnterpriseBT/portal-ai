...
- `apps/web/vite.config.ts`: a `rawD3Umd()` virtual-module plugin serving `d3/dist/d3.min.js?raw` as text — d3's package `exports` map exposes no subpaths (deep imports can't resolve), and an alias into `node_modules` feeds the dep optimizer a `?raw` asset, which crashes it and poisons the deps cache. The `\0`-virtual module resolves before both and is never optimized (dev, build, Storybook alike).
...
**`apps/web`** — new: `modules/D3Widget/` (barrel, 2 component files, 6 utils, tests, stories), `src/__tests__/raw-stub.js`; edit: `api/portal-sql.api.ts` (+`handleSnapshotPage`), `main.tsx` (registration), `jest.config.js` (`?raw` mapper), `vite.config.ts` (`rawD3Umd()` virtual-module plugin), `package.json` (+`d3` exact).
