...
For AWS this is now concrete (#397): the read-scoped credential is the `portalai-{env}-operator-read` role and the write credential is `portalai-{env}-operator-write` (prod-write MFA-gated), both assumed by the assume-only `portalai-operator` user — defined in `infra/cloudformation/iam.yml`, which also replaced `AdministratorAccess` on the `github-cicd-deploy-{dev,prod}` roles with scoped policies. See [`AWS_CLI_OPS.md`](AWS_CLI_OPS.md) → *Operator roles*.
...
| Inspect a CloudFormation stack's status / outputs | maintenance | app-dev | aws | `aws cloudformation describe-stacks --stack-name portalai-dev-backend` | yes | [#224](https://github.com/EnterpriseBT/portal-ai/issues/224) | covered |
| Deploy / update an infra stack (ad-hoc; normal path is CI) | configuration | app-dev | aws | `aws cloudformation deploy --stack-name portalai-dev-backend --template-file infra/cloudformation/backend.yml` | yes | [#224](https://github.com/EnterpriseBT/portal-ai/issues/224) | covered |
...
| Inspect the domain's DNS records | maintenance | all | aws | `aws route53 list-resource-record-sets --hosted-zone-id <zone-id>` | yes | [#369](https://github.com/EnterpriseBT/portal-ai/issues/369) | covered |
| Publish / update mail DNS (MX, SPF, DMARC, DKIM) | configuration | all | aws | `aws cloudformation deploy --stack-name portalai-dns-email --template-file infra/cloudformation/dns-email.yml` | yes | [#369](https://github.com/EnterpriseBT/portal-ai/issues/369) | covered — domain-wide, not per-env |
| Provision a mailbox / alias (Google Workspace) | configuration | all | — | `—` | no | [`docs/PORTALSAI_MAIL.runbook.md`](PORTALSAI_MAIL.runbook.md) | **out-of-band, runbook-driven** — a vendor console with a paid seat and no API we hold credentials for |
| Obtain the DKIM key after domain verification | configuration | all | — | `—` | no | [`docs/PORTALSAI_MAIL.runbook.md`](PORTALSAI_MAIL.runbook.md) | **out-of-band, runbook-driven** — generated by Google only after verification, then passed to the stack as `DkimValue` |
| Complete the deployment security review (per residency client; SaaS baseline once) | configuration | prod | — | `—` | no | [`docs/DEPLOYMENT_SECURITY_REVIEW.md`](DEPLOYMENT_SECURITY_REVIEW.md) | **out-of-band, runbook-driven (#582)** — a human review with the client's security team (egress / subprocessor DPA / telemetry / deployment checklist); the template carries the SaaS baseline as its worked example |
...
_Auth: `cli-env` — AWS-IAM (infra/DB) + Auth0 device-flow (app API); `--env` required on every command, `--json` on every command. Guards are keyed on env `kind`: `local` unrestricted, `app-dev` (staging) mutations need `--yes`, `prod` destructive **blocked** + non-destructive needs `--yes --confirm-prod`. For `--env local`, DB-touching ops (`db *`, `org`/`user`/`member`, `tier apply`) read `DATABASE_URL` (and `AUTH0_*` for `login`) from the CLI's **own package `.env`** (`packages/admin-cli/.env` / `packages/devops-cli/.env`, git-ignored, auto-loaded for local only — #632; copy from the committed `.env.example`), or from an explicit shell export (which wins). app-dev/prod ignore the file and resolve from AWS. `vars` is **`app-dev`/`prod` only** — `local` config lives in `.env`, not an AWS catalog. Full runbook: [#227](https://github.com/EnterpriseBT/portal-ai/issues/227)._

| Operation | Category | Envs | Owning CLI | Command | Operable? | Guide ref | Disposition |
|---|---|---|---|---|---|---|---|
| List env config values (masked) | configuration | app-dev | portalops | `portalops vars list --env app-dev --json` | yes | [#227](https://github.com/EnterpriseBT/portal-ai/issues/227) | covered |
| Get one config value | configuration | app-dev | portalops | `portalops vars get <KEY> --env app-dev --json` | yes | [#227](https://github.com/EnterpriseBT/portal-ai/issues/227) | covered |
...
| One-shot local provisioning after a full reset (migrate + seed + tier apply + optional e2e org) | maintenance | local | portalops | `portalops local provision --env local --yes` | yes | [#490](https://github.com/EnterpriseBT/portal-ai/issues/490) | covered (local-only by contract — other envs exit 2) |
...
**Classified:** `55 / 55` operations carry a non-blank disposition → **100% classified**, zero unclassified gaps.
...
| **Total** | **55** | **52** | **3** | **46 / 49 (93.9%)** |

Three operations are non-operable. One — inject a secret into the running ECS task — is a deploy-side wiring step (finding (a)), not a missing CLI command; its config *value* half is operable via `portalops vars set`. The other two are #369's mailbox provisioning and DKIM retrieval: **out-of-band by nature**, performed in a vendor console, and carried by [`docs/PORTALSAI_MAIL.runbook.md`](PORTALSAI_MAIL.runbook.md) rather than by a command. See [Gap list & findings](#gap-list--findings).
